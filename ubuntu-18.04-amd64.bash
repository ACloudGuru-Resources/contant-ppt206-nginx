...
puppet_config_set() {
  "${PUPPET_BIN_DIR?}/puppet" config set server puppet.ec2.internal --section main
...
mcollective_status() {
  output=$("$(puppet_bin_dir)/puppet" resource service "mcollective" 2> /dev/null)
  case ${output?} in
    *ensure*=\>*running*)
      echo "running";;
    *)
...
curl_or_wget() {
  ...
  if cmd curl; then
    ...
    if use_puppet_master_ca_cert; then
      t_ssl_options="--tlsv1 --cacert $DEFAULT_CA_CERT"
    elif curl_has_peer_verify_by_default; then
      # we need to disable it
      t_ssl_options="--tlsv1 -k"
    else
    ...
    if use_puppet_master_ca_cert; then
      t_ssl_options="--secure-protocol=TLSv1 --ca-certificate=$DEFAULT_CA_CERT"
    else
      t_ssl_options="--secure-protocol=TLSv1 --no-check-certificate"
    fi
...
supported_platform() {
  ...
  t_supported_platform_regex='(el-(5|6|7)-(i386|x86_64|s390x|ppc64le|aarch64))|(redhatfips-7-x86_64)|(debian-(8|9)-(i386|amd64))|(ubuntu-(14\.04|16\.04)-(i386|amd64|ppc64el))|(ubuntu-18\.04-amd64)|(sles-(11|12|15)-(i386|x86_64|s390x|ppc64le))|(fedora-(26|27|28|29)-(x86_64))|(solaris-(10|11)-(i386|sparc))|(aix-(6\.1|7\.1)-power)|(osx-10\.(12|13)-x86_64)'
...
run_agent_install_from_url() {
    ...
    The required agent packages should be retrieved when puppet runs on the master, after which you can run the install.bash script again."
            ...
            fail "This method of agent installation is not supported for ${PLATFORM_TAG?} in Puppet Enterprise v2018.1.7"
        fi
    fi
...
#  do
#    curl_or_wget_quietly https://puppet.ec2.internal:8140/packages/${gpg_key} | apt-key add -
#  done
#}
...
setup_repo() {
  ...
  t_packages_gz_url="https://puppet.ec2.internal:8140/packages/2018.1.7/ubuntu-18.04-amd64/Packages.gz"
  ...
  if [ -z "${t_http_code?}" -o "${t_http_code?}" != '200' ]; then
      t_deb_source="https://puppet.ec2.internal:8140/packages/2018.1.7/ubuntu-18.04-amd64 bionic puppet5"
  else
      t_deb_source="https://puppet.ec2.internal:8140/packages/2018.1.7/ubuntu-18.04-amd64 ./"
  fi
...
#  echo -e "Acquire::https::puppet.ec2.internal::Verify-Peer false;\nAcquire::http::Proxy::puppet.ec2.internal DIRECT;" > /etc/apt/apt.conf.d/90pe-repo
#  import_gpg_key
...
install_agent() {
  ...
  set -x
  DEBIAN_FRONTEND=noninteractive apt-get install -y -o Apt::Get::Purge=false -o Dpkg::Options::='--force-confold' -o Dpkg::Options::='--force-confdef' --no-install-recommends puppet-agent=5.5.16*
  set +x
